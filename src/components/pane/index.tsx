...
import { CSSProperties, ReactNode, Ref, memo, useEffect, useMemo, useRef, useState } from 'react';

import { useMergeClasses } from '../../hooks';
import { SplitType } from '../split-pane';
...
export type PaneProps = {
  ...
  collapseOverlayCss?: CSSProperties;
...
const UnMemoizedPane = ({
  ...
  const timeout = useMemo(
    () => transitionTimeout ?? DEFAULT_COLLAPSE_TRANSITION_TIMEOUT,
    [transitionTimeout]
  );
  ...
  const minStyle = useMemo(
    () => (isVertical ? { minWidth: minSize } : { minHeight: minSize }),
    [minSize, isVertical]
  );
  const widthPreserverStyle: CSSProperties = isCollapsed
...
export const Pane = memo(UnMemoizedPane);
